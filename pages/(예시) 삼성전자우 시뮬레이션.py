...
st.write('''
    - 삼성전자우/삼성전자의 시가와 종가를 각각 나누어 가격비율을 구함
    - 시가를 기준으로 진입 및 청산신호 생성
    - 종가를 기준으로 진입 및 청산 실행
    ---
    [진입 및 청산 신호]
    1) 시가 가격비율의 룩백 기간의 이동평균과 표준편차를 구함
    2) 가격비율의 시가가 지정한 표준편차 상단을 상회하면 종가에 페어 매도  
       (삼성전자우 매도, 삼성전자 매수)
    3) 가격비율의 시가가 지정한 표준편차 하단을 하회하면 종가에 페어 매수  
       (삼성전자우 매수, 삼성전자 매도)
    4) 청산은 가격비율의 시가가 평균을 터치(하향/상향)시 종가에 페어 청산        
    '''
    )
...
st.write('''
    - 파라메터 : 룩백 기간 50일, 표준편차 2배 (최적값은 아래 파라메터 조합 참조)
...
st.markdown("---")   # 구분 가로선
st.subheader("많은 파라메터 테스트 결과 참고")
st.write('''
    이동평균(MA) 길이와 표준편차(SD) 배수에 따른 샤프지수
    ''')
